...
const services = [
  ["01", "Sites profissionais", "Sites institucionais, landing pages e experiências digitais focadas em apresentar empresas, serviços e gerar oportunidades."],
  ["02", "Sistemas & plataformas", "Painéis administrativos, portais, plataformas de clientes, sistemas internos e aplicações web sob medida."],
  ["03", "Automações & integrações", "Automação de tarefas, bots, extensões, integrações entre serviços, APIs e redução de processos manuais."],
  ["04", "Software sob medida", "Ferramentas específicas para operações em que uma solução pronta simplesmente não atende ao negócio."],
]
...
export function Services() {
  ...
  return <section id="services" className="section-shell scroll-section py-24 md:py-36">
    ...
    <div className="border-b border-white/10">{services.map(([number, title, description], index) =>
      <motion.article key={title} data-cursor-hover onPointerEnter={() => setActive(index)} onPointerLeave={() => setActive(null)} initial={{ opacity: 0, x: index % 2 ? 28 : -28 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: .55 }} transition={{ duration: .65 }} className={`service-row group grid gap-4 border-t border-white/10 py-8 md:grid-cols-[90px_1fr_1fr] md:items-center md:px-5 md:py-10 ${active === index ? "is-active" : ""}`}>
        ...
        <motion.h3 animate={{ x: active === index ? 12 : 0 }} transition={{ type: "spring", stiffness: 260, damping: 25 }} className="font-sans text-3xl font-light uppercase tracking-tight md:text-5xl">{title}</motion.h3>
